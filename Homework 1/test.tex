...
\section{Question 1}
    \begin{enumerate}[label=\alph*)]
        ...
        For example, a variable might have $D=\{a,b\}$ but setting the variable to $a$ could constraint another variable such that the variable no longer has any possible assignments.
        An example of this:
        ...
        \begin{tikzpicture}[node distance=2cm, every node/.style={circle, draw}]
            ...
            \draw (B) -- node[midway, above, draw=none] {$\neq$} (C);
            \draw (C) edge [out=90,in=90] node[above, draw=none] {$\neq$} (A);

            % Add labels for the domain
            \draw (A) node [below, yshift=-0.5cm,draw=none] {$\{0, 1\}$};
            \draw (B) node [below, yshift=-0.5cm,draw=none] {$\{0, 1\}$};
            \draw (C) node [below, yshift=-0.5cm,draw=none] {$\{0, 1\}$};

        \end{tikzpicture}

        After peforming AC3 constraint propogation, the constraint graph remains the same. The domains on the variables contain values 1 and 2 but
        there is no solution to the problem at all.
    \end{enumerate}
\end{document}
